...
def test_sym_encrypt_decrypt():
    ...
    os.environ["CAIG_ENCRYPTION_SYMMETRIC_KEY"] = ""
    ...
    os.environ["CAIG_ENCRYPTION_SYMMETRIC_KEY"] = key
...
def test_config_service_easter_island_url():
    load_dotenv(override=True)
    key = ConfigService.envvar("EASTER_ISLAND_KEY")
    url = ConfigService.envvar("EASTER_ISLAND_URL")
    print("easter_island_key: {}".format(key))
    print("easter_island_url: {}".format(url))
    ...
    os.environ["CAIG_ENCRYPTION_SYMMETRIC_KEY"] = key
    assert (
        ConfigService.sym_decrypt(url) == "https://en.wikipedia.org/wiki/Easter_Island"
    )
